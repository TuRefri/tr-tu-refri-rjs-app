...
export default function LocationsToShowInMap({ map, handleCloseModal, handleSelectStoreOnMap }) {
    ...
    filteredLocations.forEach(item => {
      if (!markers.some(marker => marker.title === item.id)) {
        ...
        const marker = new google.maps.marker.AdvancedMarkerElement({
          map,
          position: { lat: item.longitud, lng: item.latitude },
          content: imgElement,
          title: item.id,
        });
  ...
  if (loading) {
    return (
      <div className='flex gap-x-2 text-sm px-6 py-2 bg-white shadow-lg text-gray-600 rounded-full items-center'>
        <div role="status">
          <svg
            aria-hidden="true"
            className="w-3 h-3 text-blue-500 animate-spin dark:text-blue-500 fill-gray-100 dark:fill-gray-100"
            viewBox="0 0 100 101"
            fill="none"
