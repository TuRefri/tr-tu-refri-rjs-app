...
export default function StoreCard( { data, authenticated}: StoreCardProps) {
    const { handleAddMagnet } = useGlobalContext()
    const navigate = useNavigate()
    const handleActionButton = (data : MagnetRefriProps) => {
        if(!authenticated){
            toast('¡Inicia sesión para poder agregar imanes a Tu Refri!', {
                action: <button 
  ...
  return (
    <article className='shadow-sm cursor-pointer w-full rounded-lg border bg-gray-50' onClick={() =>  handleActionButton(data)/* handleOpen(data) */}>
