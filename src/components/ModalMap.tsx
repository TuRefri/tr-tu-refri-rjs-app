...
export default function ModalMap({data, handleCloseModal} : ModalMapProps) {
  ...
  const magnetExistInMagnetGroup = (id : string, items : MagnetsItem[]) => {
    const existsInMagnetGroup = items.find(item => item.location.id === id)
    if(existsInMagnetGroup) return true
    return false
  }
  return (
    <motion.div
      ...
      <button className="absolute top-4 right-4 rounded-[50px] h-8 w-8 p-1 flex justify-center items-center bg-white shadow-md active:shadow-[inset_-1px_1px_5px_#cccccc,inset_1px_-1px_5px_#f9f9f9]  transition-transform duration-150" onClick={() => handleCloseModal()}>
            ...
            <section className='w-[20%] flex justify-center'>
                <div className='h-auto rounded-full overflow-hidden'>
                    <img 
                      src={`https://${awsS3Name}.s3.${awsS3Region}.amazonaws.com/${data?.store.avatarImage}`} alt={data?.store.name} className='border aspect-square object-cover mix-blend-darken'/>
                </div>
            </section>
            <section className='relative w-full'>
                <h1 className='font-medium text-lg'>{data?.store.name}</h1>
                <p className='font-light text-xs w-fit flex'>
                    {data?.store.categories.items[0].category.name}
                </p>
            </section>
        </div>
